...
def get_timing_signal_1d_given_position(channels,
  ...
  scaled_time = (
      tf.expand_dims(tf.compat.v1.to_float(position), 2) * tf.expand_dims(
          tf.expand_dims(inv_timescales, 0), 0))
  signal = tf.concat([tf.sin(scaled_time), tf.cos(scaled_time)], axis=2)
  signal = tf.pad(signal, [[0, 0], [0, 0], [0, tf.compat.v1.mod(channels, 2)]])
  return signal
...
def dot_product_attention(q, k, v, bias, dropout_rate=0.0):
  ...
  if bias is not None:
    ...
    bias = tf.compat.v1.matmul(broadcast_ones,
