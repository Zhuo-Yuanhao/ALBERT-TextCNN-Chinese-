...
def read_csv(input_file):
    ...
    lines2 = [[list(hp.dict_label.keys())[list(hp.dict_label.values()).index(l[0])], l[1]] for l in lines if
              type(l[1]) == str]
...
def file_based_input_fn_builder(input_file, seq_length, is_training,
    ...
    name_to_features = {
        "input_ids": tf.compat.v1.FixedLenFeature([seq_length * multiple], tf.int64),
        "input_mask": tf.compat.v1.FixedLenFeature([seq_length * multiple], tf.int64),
        "segment_ids": tf.compat.v1.FixedLenFeature([seq_length * multiple], tf.int64),
        "label_ids": tf.compat.v1.FixedLenFeature([], labeltype),
...
def create_model(albert_config, is_training, input_ids, input_mask, segment_ids,
    ...
    if hub_module:
        tf.compat.v1.logging.info("creating model from hub_module: %s", hub_module)
        output_layer = _create_model_from_hub(hub_module, is_training, input_ids,
                                              input_mask, segment_ids)
    else:
        tf.compat.v1.logging.info("creating model from albert_config")
        ...
        if mode == tf.estimator.ModeKeys.TRAIN:
            ...
            if task_name not in ["sts-b", "cola"]:
                ...
                def metric_fn(per_example_loss, label_ids, logits, is_real_example):
                    ...
                    concat1 = tf.sparse.concat(0,logits)
                    concat2 = tf.sparse.concat(0,label_ids)
